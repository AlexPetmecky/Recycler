...
public class InventoryListeners implements Listener {
    ...
    public InventoryListeners(Recycler plugin, HyperApi HyperApi){
        this.plugin=plugin;
    ...
    public void createRecyclerBlock(BlockDispenseEvent event){
        ...
        if(dispensedBlock.getType() ==Material.BEDROCK && event.getBlock().getType() == Material.DISPENSER){
            ...
            recyclerMeta.setDisplayName("Recycler_"+Integer.toString(len));//make sure to do len-1 when getting the position in the arraylist later
    ...
    public void moveEvent(InventoryMoveItemEvent event){
        ...
        if(recycleFrame != null){
            ...
            if(insideItem.getType() == Material.GREEN_GLAZED_TERRACOTTA){
                ...
                if(event.getItem().hasItemMeta()){
                    if(event.getItem().getItemMeta().getDisplayName().equals("Contraption Block")) {
                        ...
                        for(int i=0; i<splitLore.length;i++){
                            ...
                            if(splitLore[i].charAt(0)=='[' || splitLore[i].charAt(splitLore[i].length()-1)==']'){
                ...
                //maybe rework this part later so it happens once and the check is closer to the end?
                if(recipeWeight != 1){
                    //more than 1 item is needed to go backwards
                    //refer to the stick/plank example --> as in it takes 4 sticks to revert to 2 plank
                    //                                 --> recipeWeight here is 4
                    //if(stableStorage.containsKey())
                    //backend.shouldProduce(,);
                    //backend.shouldProduce();
                    int recyclerIndex = Integer.parseInt(recyclerName.split("_")[1]) -1;//this should get the index that the recycler is stored at in the array of recyclers


                    ArrayList<WeightedNode> parts = HyperAPI.getTargets(recipeNode.getSource());//all the itemNodes pointing to the recycler node
                    ...
                    //itemNodeWeights.put("FinalOutPutWeight",recipeWeight);//adding this so the gcd can also be found
                    //ArrayList<Integer> scaledWeights = new ArrayList<>();
                    itemNodeWeightsNumbers.add(recipeWeight);

                    //!!!!!have both a hashmap and an arraylist --> use the arraylist to find the gcd and then loop through all the hashmap and scale by the gcd!!!!
                    int gcd = HelperFunctions.GCDofList(itemNodeWeightsNumbers);//scaling factor//i may need to change this
                    ...
                    boolean shouldProduce = backend.shouldProduce(item.toLowerCase(),scaledAmountOfInput,recyclerIndex);
                    ...
                    if(shouldProduce){
                        ...
                        for (WeightedNode recipePart:recipeParts) {
                            ...
                            ItemStack output = new ItemStack(Material.getMaterial(nameChange.toUpperCase()),Math.abs(scaledWeight));
                            //System.out.println("OUTPUT: "+output);
                            event.getDestination().addItem(output);
