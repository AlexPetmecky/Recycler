...
public class RecyclerBackEnd {
    ...
    //each hashmap contains the item being recycled for example in the stick recycled to plank example -- > it contains sticks until it can produce the planks
    ...
    //A note about should produce, the hashmap being passed could very well be a completly different size with different elements
    //basically dont forget that the recyler can produce different things so it needs to keep track of that in some way; maybe store the number of sticks passing through;-->so like is they give a stick store .25 plank? or that there is one stored stick?-->probably that there is one stored stick
    public boolean shouldProduce(/*HashMap<String,Integer> storage, */String key, int valNeeded,int recyclerNumber){
        ...
        //amountsneeded is a hashmap because it has to check all the differnt ones and know which they correspond too-->no its valNeeded as an integer because it can only produce one output (crafting cannot have multiple outputs)
